...
- **Class trainer**: with Tweaks Forever loaded, "Visit your class trainer · 3 new spells" appears above the cards and in the tracker when you have spells to train. Its red X skips it for the session; right-click for *Not interested*, which *Skipped* in the cog can undo. It has no map mark, because the data has no trainer locations.
- **Something new**: when a level or a new zone brings a journey you haven't been offered, the tracker says so once ("Duskwood is now for your level"), the Adventure tab and the addon compartment get Blizzard's alert pip, and the new card is marked. Opening the tab clears them; nothing opens by itself.
