...
- **As built (commit 14).** Six changes from the bullets here, each because the bullet as written could not produce a faithful image:
  - The scenes come from `tests/scenes.lua`, which drives the same harness as `ui_spec` and prints every dump (panel, search, pins, tooltip lines, tracker, menu, route stops) as JSON. Its first run must equal `tests/golden/layout.json` byte for byte, so the golden stays the input and the fixture cannot drift. A single golden cannot hold the other five scenes.
  - A layout pass. The harness has no layout engine, so the zone cards' widths (set in `OnSizeChanged`), the scroll child's height and the route rows (placed from `GetTop`/`GetBottom`) are all unset headlessly. `screenshots.py` resolves rects, feeds them back through `h.SetRects` (which runs `OnSizeChanged`) and reruns until nothing moves. The card art comes back the same way, from the map's DB2 tiles. The harness gains laid-out `GetSize`/`GetTop`-style getters, which specs never trigger.
  - Anchors on both edges of an axis give the size; one edge plus a centre point does not (TOPLEFT + RIGHT keeps the explicit height, as Blizzard's `BNet.xml` TopLine relies on). A wrapped font string's height is its lines.
  - The dump gains what drawing needs and the client's dump omits: layers, colours, texcoords, masks, backdrops, status bars, button text and fonts, and the state of stock innards the addon touches (an inset's hidden `Bg`, a search box's instructions, the scroll bar's anchors). The harness's `GetQuestsTabAnchorOffset` now returns Camelot's `5, -28`.
  - Recipes exist for the templates the dump actually names (`QuestLogTabButtonTemplate` is not among them; the tabs are `LargeSideTabButtonTemplate`). The tracker and menu frames are wowmock's, filled with the addon's text from the harness.
  - `pip` is not installed on the machine that pinned the environment, so the header records `importlib.metadata`'s Pillow version, the wowmock sha256 and the font hashes.
...
- **Negative tests.** A vendored copy with `Cancel fun(owner: string)` (today's AGF drift, Integrations.lua:17) must fail against SPF's `fun(owner: string): boolean`, as must a missing mirror field. **Changed (commit 12):** the negative copies are derived in memory from `types/Namespace.lua` (one `gsub` each, asserted to hit exactly one line) rather than kept as more vendored files, so they cannot go stale; a `REQUIRED` list missing a function is a third negative case. The vendored SPF file is excluded from luacheck and StyLua so it stays byte-identical to upstream.
...
| 17 | Click the second card, then hover each one-line row | The other two fold to 28 px rows above it; the chosen card is pressed and lit with its steps right under it; each row's tooltip has its subline and reason | SHOT panel |
