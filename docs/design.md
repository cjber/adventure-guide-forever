...
- **Card kinds** (three at most, only those that have steps). Carry and the story keep their fixed slots; the
  ...
  3. **Diversions**, newest first:
     - **Your calling** (roadmap #7): the class quests the player can take now (a `classes` mask that is not every
       class), as one card. It leads with a chain as the story does (§2.3), else the lowest quest ID. Its reason names
