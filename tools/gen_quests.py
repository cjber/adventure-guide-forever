...
def geometry(assignments, wanted):
    """Each map's continent (the world MapID) and the world-coordinate centre of its rectangle, so the planner can
    rank steps on different maps without asking Shortest Path. A map without exactly one whole-map row is left out.
    """
...
def generate(tables, ui_maps, assignments, valid_ids):
    ...
    centres = geometry(assignments, wanted)
    counts["maps without a centre"] = len(wanted - centres.keys())
    return emitted, zones, centres, counts
...
def render(quests, zones, centres):
    lines = [
...
def main():
    ...
    quests, zones, centres, counts = generate(
        tables,
    ...
    OUTPUT.write_text(render(quests, zones, centres), encoding="utf-8")
