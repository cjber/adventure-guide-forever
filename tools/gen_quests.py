...
def generate(tables, ui_maps, assignments, valid_ids, path_nodes, taxi_nodes):
    ...
    return emitted, zones, centres, shifts, ferries, counts
...
def render(quests, zones, centres, shifts, ferries):
    lines = [
        "-- Generated by tools/gen_quests.py — do not edit.",
        f"-- CMaNGOS classic-db (GPL-3.0), pinned: {CLASSICDB_URL}",
        "-- wago.tools UiMap, UiMapAssignment, QuestV2, TaxiPathNode, TaxiNodes:",
...
def main():
    ...
    quests, zones, centres, shifts, ferries, counts = generate(
        tables,
        ...
        db2("TaxiNodes", ("ContinentID", "Pos_0", "Pos_1", "Flags"), **options),
    )
    if not quests or not counts["with start"]:
        raise ValueError("No usable quests; leaving existing output untouched")
    OUTPUT.parent.mkdir(parents=True, exist_ok=True)
    OUTPUT.write_text(render(quests, zones, centres, shifts, ferries), encoding="utf-8")
