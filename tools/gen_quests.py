...
def main():
    ...
    OUTPUT.write_text(render(quests, zones), encoding="utf-8")
    for name, count in sorted(counts.items()):
        print(f"{name}: {count}")
    print(
        f"Wrote {len(quests)} quests, {len(zones)} zones: {OUTPUT.relative_to(ROOT)} ({OUTPUT.stat().st_size:,} bytes)"
    )
